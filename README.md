...
- CI bumps Locust’s default load (`LOCUST_USERS=150`, `LOCUST_SPAWN_RATE=30`) and runs `scripts/validate_chaos_live.py` up front (60 s chaos window with a 15 s delay, retries until ≥200 requests recorded) to prove что контейнеры реально глушатся и `R_live` опускается ниже порога; результаты попадают в `validation_*.json`.
