...
## Notes
- Мы **держим демо-Локуст** и измеряем `R_live = 1 - (5xx + transport errors + Locust #failures)/total`, а также прогоняем 100 фронтовых проб (включая полный сценарий `cart → checkout`). Любая неудача в этих пробах “режет” окно, чтобы end-to-end ошибки вроде checkout тоже учитывались. В CI добавлен baseline health check (без хаоса) — если он не проходит, пайплайн останавливается ещё до экспериментов.  
